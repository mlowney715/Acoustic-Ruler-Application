...
configPrefPath = os.path.realpath(os.path.abspath(os.path.join(os.path.split(inspect.getfile( inspect.currentframe() ))[0],"configPref")))
...
##############################################################################################################   
##############################################################################################################
class ConfigureDeviceSngChanWindow(wx.Dialog):
...
#------------------------------------------------------------------------------------------------
    def serial_ports(self,event):
        if WinOsName == GetOsName:
            tempList = ['COM%s' % (i + 1) for i in range(256)]
        elif LinuxOsName == GetOsName:
            tempList = glob.glob('/dev/tty[A-Za-z]*')
        elif MacOsName == GetOsName:
...
#-------------------------------------------------------------------------------------------------------------------
    def configDeviceBtnClicked(self,event):
        configDevFrame = ConfigureDeviceSngChanWindow(parent=self,ID=996)
